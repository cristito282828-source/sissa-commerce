...
export default function CategorySectionDynamic({ collections }: CategorySectionDynamicProps) {
  ...
  const isUncategorized = (val?: string) => {
    const n = normalize(val);
    return (
      n.includes('sincategorizar') ||
      n.includes('sincategoria') ||
  ...
  const validCollections = collections
    .filter((c) =>
      c.handle !== '' &&
      !normalize(c.handle).includes('hidden') &&
      normalize(c.title) !== 'all' &&
      !isUncategorized(c.handle) &&
      !isUncategorized(c.title)
    )
    .map(c => ({
      ...c,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 sm:gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-white/10 text-white text-sm font-semibold uppercase tracking-[0.22em]">
            Logo
          </div>
          </div>
                    ...
                    >
                      ...
                      <div className="relative aspect-[4/6] w-full overflow-hidden bg-gray-200 rounded-sm">
                          <Image
                            ...
                            alt={`Categoría ${category.title} - Explora productos de ${category.title}`}
                            fill
